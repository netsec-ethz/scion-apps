...
func ReadTracerouteItemsSince(since string) ([]TracerouteGraph, error) {
	...
	ORDER BY datetime(a.Inserted), h.Ord DESC
    `
	...
	for _, hop := range result {
		if lastEntryInsertedTime != hop.Inserted {
			trg := TracerouteGraph{}
			trg.Inserted = hop.Inserted
			trg.ActualDuration = hop.ActualDuration
			trg.CmdOutput = hop.CmdOutput
			trg.Error = hop.Error
		...
		rth := ReducedTrHopItem{}
		rth.HopIa = hop.HopIa
		rth.HopAddr = hop.HopAddr
		rth.IntfID = hop.IntfID
		rth.RespTime1 = hop.RespTime1
		rth.RespTime2 = hop.RespTime2
		rth.RespTime3 = hop.RespTime3
		
		lastEntry := graphEntries[len(graphEntries) - 1]
		lastEntry.TrHops = append(lastEntry.TrHops, rth)
	}
